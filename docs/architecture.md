...
Нічого. `src/pipeline/` (pipecat, ~30 стадій) і `src/core/` були
видалені 17 серпня; порожні теки прибрані 25-го. Шлях відкоту, заради
якого їх тримали — `engine = "pipecat"` у `config.toml` — зник разом із
ними: жоден рядок коду не читає це значення, і в `Settings` немає
такого поля. Рядок `engine = "spine"` у живому конфігу теж не робить
нічого.
