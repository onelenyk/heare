"""Runtime-switchable LLM service between OpenRouter and z.ai."""
...
class SwitchableLLMService(LLMService):
    """LLM service that hot-swaps between OpenRouter (OpenAI shape) and z.ai
    (Anthropic shape) at runtime without rebuilding the pipeline.

    Architecture: composition over inheritance.
        - Holds two fully-formed Pipecat delegates: ``_or_service``
          (``OpenAILLMService``) and ``_zai_service`` (``AnthropicLLMService``).
    ...
    Provider routing:
        - ``~/.heare/provider`` (a single text file containing
          ``openrouter`` or ``zai``) is the source of truth.
    ...
    Tool registration:
        - ``register_function``/``unregister_function`` fan out to BOTH
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not openrouter_api_key and not zai_api_key:
            raise ValueError(
                "At least one provider key (OPENROUTER_API_KEY or ZAI_API_KEY) must be set"
            )

        if openrouter_api_key and not zai_api_key:
            logger.info("switchable_llm: z.ai disabled (ZAI_API_KEY unset)")
            self._active_provider = "openrouter"
        elif zai_api_key and not openrouter_api_key:
            logger.info(
                "switchable_llm: OpenRouter disabled (OPENROUTER_API_KEY unset); "
                "defaulting to z.ai"
            )
    ...
    def _sync_provider(self) -> str:
        ...
        try:
            ...
            if raw == "zai":
                if self._zai_service is None or self._zai_disabled:
                    self._active_provider = "openrouter"
                else:
                    self._active_provider = "zai"
                    logger.info("switchable_llm: switched to z.ai provider (configured)")
            else:
                if self._or_service is None:
                    self._active_provider = "zai"
                else:
                    self._active_provider = "openrouter"
    ...
    def active_provider(self) -> str:
        """Return the currently active provider ('openrouter' or 'zai')."""
        self._sync_provider()
        return self._active_provider
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, (OpenAILLMContextFrame, LLMContextFrame, LLMMessagesFrame)):
            if not self._turn_in_flight:
                ...
                key = "zai" if self._turn_delegate is self._zai_service else "or"
                ...
                model = self._zai_model if self._active_provider == "zai" else self._or_model
