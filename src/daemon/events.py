    ...
    if _events_file:
        try:
            with open(_events_file, "a") as f:
                f.write(json.dumps({"stage": stage, "event": event, "level": level, "ts": e.ts, "data": data}) + "\n")
        except Exception:
            pass

def rate_limited(stage: str, event: str, key: str, interval: float = 5.0, level: str = "info", **data) -> bool:
...
def recent(limit: int = 50) -> list[dict]:
    ...
    return [
        {"stage": e.stage, "event": e.event, "level": e.level, "ts": e.ts, "data": e.data}
