...
async def _cmd_start(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        pipeline, decider = await build_pipeline(
        ...
        await run_until_stopped(runner, pipeline, heartbeat)
    finally:
...
async def run_until_stopped(runner, pipeline, heartbeat) -> None:
    ...
    try:
            ...
            {pipeline_task, heartbeat_task, stop_waiter},
            return_when=asyncio.FIRST_COMPLETED,
        )
    finally:
        heartbeat.stop()
        for task in (pipeline_task, heartbeat_task, stop_waiter):
            if not task.done():
                task.cancel()
        for task, name in ((pipeline_task, "pipeline"), (heartbeat_task, "heartbeat")):
            try:
                await task
