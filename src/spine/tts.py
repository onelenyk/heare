...
def apply_fade_out(pcm: bytes, sample_rate: int, fade_ms: int = 50) -> bytes:
    """Apply a linear-ramp fade-out to the last ``fade_ms`` of PCM s16le mono.

    PCM is signed 16-bit little-endian mono; the last ``fade_ms`` of
    samples (or the entire buffer if shorter) are multiplied by a linear
    1.0 -> 0.0 ramp. Returns a new bytes object; input is not mutated.
