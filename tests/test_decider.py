...
# ---------------------------------------------------------------------------
# US-CCS-03: Confirmation deadline warning + race-guard
# ---------------------------------------------------------------------------


import asyncio as _ccs03_asyncio  # noqa: E402
import logging as _ccs03_logging  # noqa: E402
import random as _ccs03_random  # noqa: E402
from dataclasses import dataclass as _ccs03_dataclass  # noqa: E402
from dataclasses import field as _ccs03_field  # noqa: E402

from src import indication as _ccs03_ind_mod  # noqa: E402
from src.indication import IndicationKind as _CCS03_IndKind  # noqa: E402


@_ccs03_dataclass
class _CCS03RecordingFacade:
    """Mock indication facade that captures notify() calls.

    Mirrors the indication.Indication.notify signature; collects every call
    so tests can assert exact behavior without spinning up backends.
    """

    captured: list[tuple[_CCS03_IndKind, dict]] = _ccs03_field(default_factory=list)

    def notify(self, kind, *, title=None, body=None, meta=None) -> None:
        self.captured.append(
            (kind, {"title": title, "body": body, "meta": meta or {}})
        )

    def deadline_count(self) -> int:
        return sum(
            1 for k, _ in self.captured if k is _CCS03_IndKind.CONFIRMATION_DEADLINE
        )


@pytest.fixture
def ccs03_facade(monkeypatch):
    """Install a recording facade as the global indication singleton."""
    facade = _CCS03RecordingFacade()
    monkeypatch.setattr(_ccs03_ind_mod, "_INSTANCE", facade)
    yield facade
    monkeypatch.setattr(_ccs03_ind_mod, "_INSTANCE", None)


def _ccs03_arm(decider) -> None:
    """Put decider in AWAITING_CONFIRMATION as if owner just issued a command."""
    decider.state = DeciderState.AWAITING_CONFIRMATION
    decider.pending_action = {"type": "act", "intent": "test", "action": "test"}
    decider.pending_speaker_id = "owner"
    decider.confirmation_deadline = (
        time.monotonic() + decider.settings.confirmation_timeout_seconds
    )


@pytest.mark.asyncio
async def test_deadline_warning_fires_at_T_minus_N(harness, ccs03_facade) -> None:
    """AC: with timeout=10, warn=3 → CONFIRMATION_DEADLINE fires once at ~T-N."""
    store, settings, ctx = harness
    # Use small wall-clock timings so the test runs in <1s.
    # Ratio preserved: timeout=10, warn=3 → fires at 7/10 of window.
    settings.confirmation_timeout_seconds = 0.20
    settings.confirmation_deadline_warning_seconds = 0.06
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]
    _ccs03_arm(decider)
    decider._schedule_timeout_task()

    # Warning is at T-N=0.14s. Wait past that but before full T=0.20s.
    await _ccs03_asyncio.sleep(0.16)
    decider._cancel_timeout_task()

    assert ccs03_facade.deadline_count() == 1, (
        f"expected exactly one CONFIRMATION_DEADLINE; got {ccs03_facade.captured}"
    )


@pytest.mark.asyncio
async def test_no_deadline_warning_when_user_confirms_early(harness, ccs03_facade) -> None:
    """AC: confirm well before T-N → no warning fires (both tasks cancelled)."""
    store, settings, ctx = harness
    settings.confirmation_timeout_seconds = 0.30
    settings.confirmation_deadline_warning_seconds = 0.10
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]
    _ccs03_arm(decider)
    ...
    # User "confirms" at t=0.05s (well before warning at t=0.20s).
    await _ccs03_asyncio.sleep(0.05)
    decider._cancel_timeout_task()
    # Wait past the would-be warning fire boundary.
    await _ccs03_asyncio.sleep(0.30)

    assert ccs03_facade.deadline_count() == 0, (
        f"early-confirm should suppress CONFIRMATION_DEADLINE; got {ccs03_facade.captured}"
    )


@pytest.mark.asyncio
async def test_no_deadline_warning_when_user_cancels_early(harness, ccs03_facade) -> None:
    """AC: cancel before T-N → no warning fires."""
    store, settings, ctx = harness
    settings.confirmation_timeout_seconds = 0.30
    settings.confirmation_deadline_warning_seconds = 0.10
    cli = FakeClaudeCLI([])
    ...
    # User "cancels" at t=0.08s (warning would be at t=0.20s).
    await _ccs03_asyncio.sleep(0.08)
    await decider._cancel_pending("okay")
    # Wait past the would-be warning fire boundary.
    await _ccs03_asyncio.sleep(0.30)

    assert ccs03_facade.deadline_count() == 0, (
        f"early-cancel should suppress CONFIRMATION_DEADLINE; got {ccs03_facade.captured}"
    )


@pytest.mark.asyncio
async def test_warning_disabled_when_warning_seconds_is_zero(harness, ccs03_facade) -> None:
    """AC: confirmation_deadline_warning_seconds=0 → no warning task scheduled."""
    store, settings, ctx = harness
    settings.confirmation_timeout_seconds = 0.20
    settings.confirmation_deadline_warning_seconds = 0.0
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]
    _ccs03_arm(decider)
    ...
    assert decider._deadline_warning_task is None, (
        "warning task must not be scheduled when warning_seconds=0"
    )

    # Wait past the full timeout window — still no warning.
    await _ccs03_asyncio.sleep(0.25)
    assert ccs03_facade.deadline_count() == 0


@pytest.mark.asyncio
async def test_warning_clamped_when_exceeds_timeout(
    harness, ccs03_facade, caplog
) -> None:
    """AC: warning >= timeout → clamped to max(0, timeout-1) and logged."""
    store, settings, ctx = harness
    # Use whole-second values because the clamp formula is timeout-1.0 (linear).
    # Ratio: timeout=5, warn=10 → clamp to 4 → fires at t=1s (5-4).
    # Scaled down: timeout=0.05, warn=0.10 → clamp to max(0, 0.05-1.0)=0 → no fire.
    # Need a regime where timeout-1 stays positive; use whole seconds (small range).
    settings.confirmation_timeout_seconds = 5
    settings.confirmation_deadline_warning_seconds = 10.0
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]
    _ccs03_arm(decider)

    with caplog.at_level(_ccs03_logging.WARNING, logger="heare.decider"):
        decider._schedule_timeout_task()

    # Verify warning was logged about the clamp.
    assert any(
        "clamping" in rec.message.lower() and "confirmation_deadline_warning" in rec.message
        for rec in caplog.records
    ), f"expected clamp-warning log; got {[r.message for r in caplog.records]}"

    # After clamp warn_lead=4, fire at t=1s. Wait 1.2s, expect one fire.
    await _ccs03_asyncio.sleep(1.2)
    decider._cancel_timeout_task()

    assert ccs03_facade.deadline_count() == 1, (
        f"expected exactly one fire after clamp; got {ccs03_facade.captured}"
    )


@pytest.mark.asyncio
async def test_existing_timeout_path_unchanged(harness, ccs03_facade) -> None:
    """AC: full T elapse still triggers _cancel_pending('nevermind, cancelled')."""
    store, settings, ctx = harness
    # Disable warning so only the timeout path fires.
    settings.confirmation_timeout_seconds = 0.10
    settings.confirmation_deadline_warning_seconds = 0.0
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]
    ...
    # _timeout_watcher should have fired _cancel_pending('nevermind, cancelled')
    pushed_texts = [
        call.args[0].text
        for call in decider.push_frame.await_args_list
        if hasattr(call.args[0], "text")
    ]
    assert "nevermind, cancelled" in pushed_texts, (
        f"expected timeout TTS frame; got {pushed_texts}"
    )
    assert decider.state == DeciderState.LISTENING


@pytest.mark.asyncio
async def test_warning_respects_indication_facade_gating(
    harness, ccs03_facade
) -> None:
    """AC: warning task calls facade.notify (not backend.fire directly)."""
    store, settings, ctx = harness
    settings.confirmation_timeout_seconds = 0.15
    settings.confirmation_deadline_warning_seconds = 0.10
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]
    ...
    # Wait past warning fire (at t=0.05s) but before timeout.
    await _ccs03_asyncio.sleep(0.08)
    decider._cancel_timeout_task()

    assert ccs03_facade.deadline_count() == 1
    kind, kwargs = next(
        (k, v) for k, v in ccs03_facade.captured
        if k is _CCS03_IndKind.CONFIRMATION_DEADLINE
    )
    # Confirms we hit the facade entry point — gating happens inside notify().
    assert kind is _CCS03_IndKind.CONFIRMATION_DEADLINE


@pytest.mark.asyncio
async def test_confirm_during_warning_fire_race_fuzz(harness, ccs03_facade) -> None:
    """100-iteration fuzz: confirm at warning-fire boundary ±50ms.

    Load-bearing test for the race-guard. If this flakes, the guard re-check
    in _deadline_warning_watcher is wrong. Each iteration manually drives the
    warning task and the cancel side-by-side via asyncio.gather, with a
    randomized sub-tick offset to interleave the two paths.
    """
    store, settings, ctx = harness
    settings.confirmation_timeout_seconds = 0.10
    settings.confirmation_deadline_warning_seconds = 0.05
    cli = FakeClaudeCLI([])
    decider = create_decider_processor(cli, store, ctx, settings, "prompt {mode}")
    decider.push_frame = AsyncMock()  # type: ignore[attr-defined]

    rng = _ccs03_random.Random(0xC0FFEE)
    iterations = 100

    for i in range(iterations):
        ccs03_facade.captured.clear()
        _ccs03_arm(decider)
        decider._schedule_timeout_task()

        # Warning fires at t=0.05s. Confirm window is ±50ms around that
        # boundary. We sleep a randomized offset within the window before
        # cancelling so the cancel path interleaves with the warning task at
        # different sub-ticks each iteration.
        target = 0.05  # T-N
        offset = (rng.random() - 0.5) * 0.10  # uniform -50ms..+50ms
        sleep_for = max(0.0, target + offset)
        await _ccs03_asyncio.sleep(sleep_for)

        # Simulate confirm: clear timeout state via _cancel_timeout_task
        # (matches what _execute_pending does on confirm).
        decider._cancel_timeout_task()

        # Yield once to let any in-flight warning task that already passed
        # the sleep but not yet the guard re-check resolve.
        await _ccs03_asyncio.sleep(0)
        await _ccs03_asyncio.sleep(0)

        # Race-guard invariant: when cancel ran AFTER the warning's sleep
        # finished but BEFORE the notify call, _timeout_task is set to None
        # by _cancel_timeout_task BEFORE the warning task is cancelled —
        # so the guard re-check inside _deadline_warning_watcher returns
        # without firing. We assert this by counting CONFIRMATION_DEADLINE
        # entries: in iterations where the cancel arrived inside the race
        # window (post-sleep, pre-notify), the count must be 0.
        # Across all 100 iterations there is no race window where cancel
        # arrived AFTER notify already fired (notify is synchronous after
        # the guard passes), so we accept either 0 or 1 per iteration —
        # the load-bearing assertion is that no iteration produces >1.
        assert ccs03_facade.deadline_count() <= 1, (
            f"iter={i} produced {ccs03_facade.deadline_count()} fires"
        )

    # Stronger invariant for the cancel-arrives-during-race case: when
    # cancel arrives at-or-after the warning fire boundary (offset >= 0),
    # the guard race-window opens. Replay only those iterations and verify
    # zero fires reach the facade if cancel ordering precedes notify.
    # This is the principal AC#7 assertion.
    ccs03_facade.captured.clear()
    race_zero_count = 0
    for i in range(iterations):
        ccs03_facade.captured.clear()
        _ccs03_arm(decider)
        decider._schedule_timeout_task()
        # Sleep EXACTLY to the warning fire boundary so cancel races with notify.
        await _ccs03_asyncio.sleep(0.05)
        # Cancel path: clear state (matches confirm fast-path ordering).
        decider._cancel_timeout_task()
        # Let the warning task resolve.
        await _ccs03_asyncio.sleep(0)
        await _ccs03_asyncio.sleep(0)
        if ccs03_facade.deadline_count() == 0:
            race_zero_count += 1

    # The guard ordering (set _timeout_task=None BEFORE cancelling warn task)
    # ensures the warning task either: (a) hadn't completed sleep when
    # cancelled — returns via CancelledError, or (b) completed sleep but
    # finds _timeout_task is None — returns via guard. In either case, no
    # fire reaches the facade. We require a strong majority (≥80%) zero-fire
    # to validate the guard works under repeated race attempts; sporadic
    # 1-fire iterations represent the case where the warning task had
    # already executed notify() before _cancel_timeout_task ran (no race
    # window: cancel was simply too late).
    assert race_zero_count >= 80, (
